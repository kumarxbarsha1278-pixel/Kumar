...
async def run_binary(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text("🔄 Running process (this might take a few minutes)...")
    ...
    try:
        ...
        # Execute system process with an extended 5-minute timeout
        result = subprocess.run(
            command, 
            capture_output=True, 
            text=True, 
            timeout=400  # Increased from 30 to 300 seconds
        )
        ...
        await update.message.reply_text("❌ Error: Process execution exceeded the 5-minute maximum timeout.")
